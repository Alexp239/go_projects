...
func (player *Player) GoRoom(par ...string) string {
	...
	for _, path := range player.position.paths {
		if path.room.name == roomName {
			...
			player.position.players[player.id] = player
...
func (player *Player) Apply(par ...string) string {
	if len(par) < 2 {
		return "Команда применить [предмет] [замок]"
	}
	r := player.position
	r.mu.Lock()
	defer r.mu.Unlock()
...
// SayPlayer - сказать игроку
func (player *Player) SayPlayer(par ...string) {
	if len(par) < 1 {
		player.HandleOutput("сказать_игроку [игрок] [сообщение]")
		return
	}
	r := player.position
	r.mu.Lock()
	...
	for _, pl := range r.players {
		if pl.name == par[0] {
			if ok {
				player.HandleOutput("тут несколько игроков с такими именами")
				return
			}
			reciever = pl
			ok = true
